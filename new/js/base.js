...
//下拉菜单控件 select-1
function select(a,b){
	var minWidth = $(a +" .select-"+b).width();
		minWidth = minWidth - 2;
//		console.log(minWidth)
	var display = "";
	$(a +" .select-"+ b +" .op").css("min-width",minWidth);
	$(a +" .select-"+ b).click(function(event){
		event.stopPropagation();
		display = $(a +" .select-"+ b +" ul").css("display");
		if(display=="block"){
	...
	$(a +" .select-"+ b +" .op li").click(function(){
		$(this).parent().find("li").removeClass("active");
		$(this).addClass("active");
		var textInfo = $(this).text();
		$(a +" .select-"+ b +" .select").text(textInfo);
	});
	
	$(document).click(function(event){
		var _con = $(a +" .select-"+ b +" .op");
