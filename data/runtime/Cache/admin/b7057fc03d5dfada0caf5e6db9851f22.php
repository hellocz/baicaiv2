...
<div class="pad_10" >
    ...
    <div class="J_tablelist item_imglist clearfix">
        <?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$val): $mod = ($i % 2 );++$i;?><div class="item fl">
           ...
           <div class="img clearfix"> <a href="<?php echo u('item/edit', array('id'=>$val['id'], 'menuid'=>$menuid));?>"><img src="<?php echo attach($val['img'], 'item');?>"></a></div>
            </label>
            <span class="line_x"><a href="<?php echo u('item/edit', array('id'=>$val['id'], 'menuid'=>$menuid));?>"><?php echo ($val["title"]); ?></a></span>
            <ul>
                <li style="width:auto"><a class="J_tooltip btn_blue" title="<?php echo ($cate_list[$val['cate_id']]); ?>"><?php echo L('cate');?></a></li>
                <li style="width:auto"><a class="J_tooltip btn_blue" title="<?php echo (($val["uname"])?($val["uname"]):L('item_no_author')); ?>"><?php echo L('author');?></a></li>
				<li style="width:auto"><a href="javascript:;" class="J_showdialog btn_blue" data-uri="/?g=admin&m=item&a=set_score&id=<?php echo ($val["id"]); ?>" data-title="设置积分信息" data-id="edit" data-acttype="ajax" data-width="300" data-height="140" title="审核">审核</a></li>
            </ul>
