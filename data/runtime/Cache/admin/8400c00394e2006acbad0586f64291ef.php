    ...
    <table width="100%" cellspacing="0">
        <thead>
            <tr>
				...
				<th width="40"><span data-tdtype="order_by" data-field="isbest">精华</span></th>
				<th width="40"><span data-tdtype="order_by" data-field="istop">置顶</span></th>
				<th width="40"><span data-tdtype="order_by" data-field="ishot">热门</span></th>
        ...
        <tbody>
            <?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$val): $mod = ($i % 2 );++$i;?><tr>
				...
				<td align="center"><img data-tdtype="toggle" data-id="<?php echo ($val["id"]); ?>" data-field="isbest" data-value="<?php echo ($val["isbest"]); ?>" src="__STATIC__/images/admin/toggle_<?php if($val["isbest"] == 0): ?>disabled<?php else: ?>enabled<?php endif; ?>.gif" /></td>
				<td align="center"><img data-tdtype="toggle" data-id="<?php echo ($val["id"]); ?>" data-field="istop" data-value="<?php echo ($val["istop"]); ?>" src="__STATIC__/images/admin/toggle_<?php if($val["istop"] == 0): ?>disabled<?php else: ?>enabled<?php endif; ?>.gif" /></td>
				<td align="center"><img data-tdtype="toggle" data-id="<?php echo ($val["id"]); ?>" data-field="ishot" data-value="<?php echo ($val["ishot"]); ?>" src="__STATIC__/images/admin/toggle_<?php if($val["ishot"] == 0): ?>disabled<?php else: ?>enabled<?php endif; ?>.gif" /></td>				
                <td align="center"><?php if($val["status"] == 2): ?><font color="orange">草稿</font><?php elseif($val["status"] == 3): ?><font color="orange">退回</font><?php else: ?><img  src="__STATIC__/images/admin/toggle_<?php if($val["status"] == 0): ?>disabled<?php else: ?>enabled<?php endif; ?>.gif" /><?php endif; ?></td>
                <td align="center">
				<!--<?php if($val['status'] == '0'): ?><a href="javascript:;" class="J_showdialog" data-uri="/?g=admin&m=article&a=set_score&id=<?php echo ($val["id"]); ?>" data-title="设置积分信息" data-id="edit" data-acttype="ajax" data-width="300" data-height="140" title="审核">审核</a> |<?php endif; ?>-->
				<a href="<?php echo u('article/edit', array('id'=>$val['id'], 'menuid'=>$menuid));?>">审核</a> | <a href="javascript:void(0);" class="J_confirmurl" data-acttype="ajax" data-uri="<?php echo u('article/delete', array('id'=>$val['id']));?>" data-msg="<?php echo sprintf(L('confirm_delete_one'),$val['title']);?>"><?php echo L('delete');?></a>|<a href="<?php echo u('article/fuzhi', array('id'=>$val['id'], 'menuid'=>$menuid));?>">复制</a> </td>
