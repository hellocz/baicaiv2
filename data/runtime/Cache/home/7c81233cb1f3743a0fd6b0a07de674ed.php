...
<div class="w_bl_bd">
 <div class="w_zh_dq1">
	  <a href="/" title="首页">首页</a>><a href="<?php echo U('exchange/index');?>" title="礼品兑换" class="w_zh_dq1_1">礼品兑换</a> 
	</div>
  <div class="w_jszx_1">
    <div class="w_jszx_h">
	<a href="<?php echo U('exchange/index');?>" ><span <?php if($cid == ''): ?>class="w_js_qd"<?php endif; ?>>全部</span></a>
	<?php $tag_scoreitem_class = new scoreitemTag;$cate_list = $tag_scoreitem_class->cate(array('type'=>'cate','return'=>'cate_list','cache'=>'0',)); if(is_array($cate_list)): $i = 0; $__LIST__ = $cate_list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$cate): $mod = ($i % 2 );++$i;?><a href="<?php echo U('exchange/index',array('cid'=>$cate['id']));?>" title="<?php echo ($cate["name"]); ?>"> <span <?php if($cid == $cate['id']): ?>class="w_js_qd"<?php endif; ?>><?php echo ($cate["name"]); ?></span></a><?php endforeach; endif; else: echo "" ;endif; ?>
	</div>
    <ul   id="con_jszx_1" >
	  <?php if(is_array($item_list)): $i = 0; $__LIST__ = $item_list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$item): $mod = ($i % 2 );++$i;?><li>
       <a href="<?php echo U('exchange/detail', array('id'=>$item['id']));?>" title="<?php echo ($item["title"]); ?>">
         <img src="<?php echo attach($item['img'], 'score_item');?>" title="<?php echo ($item["title"]); ?>" alt="<?php echo ($item["title"]); ?>"/><br/>
         <h2><?php echo ($item["title"]); ?></h2>
         <p>金币 <?php echo ($item["coin"]); ?></p>
         <p>剩余 <?php echo ($item["stock"]); ?> 件</p>
       </a>
      </li><?php endforeach; endif; else: echo "" ;endif; ?>
    </ul>
