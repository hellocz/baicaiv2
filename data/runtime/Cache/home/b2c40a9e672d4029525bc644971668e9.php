...
<div class="w_gezx_ce"> 
  <div class="w_ger">
	...
	<div class="w_wdcy">
	 <h2>我的菜友</h2> 
     <div class="w_cy_t">
       <span class="w_cy_t_s" id="cy1" >我关注的</span><a href="<?php echo U('user/fans');?>"><span  id="cy2" >关注我的</span></a>       
     </div>  
     <ul class="w_cygz"  id="con_cy_1">
	<?php if(is_array($user_list)): $i = 0; $__LIST__ = $user_list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$val): $mod = ($i % 2 );++$i; if($i%3 == 1): ?><li><?php endif; ?>
		<dl>
          <dt><a href="<?php echo U('space/index', array('uid'=>$val['id']));?>" title="<?php echo ($val["username"]); ?>"><img src="<?php echo avatar($val['id'], 48);?>" title="<?php echo ($val["username"]); ?>" alt="<?php echo ($val["username"]); ?>"/></a></dt>
          <dd class="w_cy_h"><a href="<?php echo U('space/index', array('uid'=>$val['id']));?>" title="<?php echo ($val["username"]); ?>"><?php echo ($val["username"]); ?></a></dd>
          <dd><a href="" title="">爆料：<i><?php echo ($val['shares']); ?></i></a>|<a href="" title="">评论：<i>366</i></a></dd>
          <dd class="J_follow_bar" data-id="<?php echo ($val["id"]); ?>"><a href="javascript:;" title="取消关注" class="J_unfo qxgz">取消关注</a></dd>
        </dl>
	 <?php if($i%3 == 0 or $i == count($user_list)): ?></li><?php endif; endforeach; endif; else: echo "" ;endif; ?>
     </ul>
