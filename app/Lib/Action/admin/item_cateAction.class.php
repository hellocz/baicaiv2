...
class item_cateAction extends backendAction {
    ...
    public function index() {
        ...
        $array = array();
        foreach($result as $r) {
            $r['str_img'] = $r['img'] ? '<span class="img_border"><img src="http://img.baicaio.com/'.attach($r['img'], 'item_cate').'" style="width:26px; height:26px;" class="J_preview" data-bimg="http://img.baicaio.com/'.attach($r['img'], 'item_cate').'" /></span>' : '';
    ...
    public function ajax_getchilds() {
       ...
       $type = $this->_get('type', 'intval', null);
        ...
        if ($return) {
            ...
            $this->ajaxReturn(0, L('operation_failure'),$return);
        }
    }
