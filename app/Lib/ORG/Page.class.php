...
/**
    ...
    public function jshow() {
        ...
        $downRow        =   $this->nowPage+1;
        if ($upRow>0){
            $upPage     =   "<a url='".str_replace('__PAGE__',$upRow,$url)."'>上一页</a>";
        }else{
            $upPage     =   '';
        }

        if ($downRow <= $this->totalPages){
            $downPage   =   "<a url='".str_replace('__PAGE__',$downRow,$url)."'>下一页</a>";
        }else{
            $downPage   =   '';
        }
        ...
        if ($this->totalPages > $this->rollPage) {
            if($this->nowPage - $middle < 1){
                ...
                $theFirst   =   "<a url='".str_replace('__PAGE__',1,$url)."' >1</a> <i>...</i>";
            }
            if($this->nowPage + $middle > $this->totalPages){
                $theEnd     =   '';
            }else{
                $theEndRow  =   $this->totalPages;
                $theEnd     =   "<i>...</i> <a url='".str_replace('__PAGE__',$theEndRow,$url)."' >".$theEndRow."</a>";
            }
        }
        ...
        if ($this->totalPages != 1) {
            ...
            for ($page = $start; $page <= $end; $page++) {
                if ($page != $this->nowPage) {
                    $linkPage .= " <a url='".str_replace('__PAGE__',$page,$url)."'>&nbsp;".$page."&nbsp;</a>";
                } else {
                    $linkPage .= " <a class='w_pag_1'>".$page."</a>";
                }
            }
