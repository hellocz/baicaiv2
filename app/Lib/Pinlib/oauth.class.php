...
class oauth {
    ...
    public function callbackLogin($request_args) {
     ...
     public function callbackLogin_m($request_args) {
        $user = $this->_om->getUserInfo($request_args);
        $bind_user = $this->_checkBind($this->_type, $user['keyid']);
        if ($bind_user) {
            //已经绑定过则更新绑定信息 自动登陆
            ...
            //把第三方的数据存到COOKIE
            cookie('user_bind_info', $user);
            return U('wap/user/binding'); //跳转到绑定注册页
        }
    }

    public function callbackSetToken($request_args){
         $token = $this->_om->getToken($request_args);
         $data['token'] =$token['access_token'];
           M('oauth')->where(array('code'=>'sina'))->save($data);
        //   return $result;
        return U('oauth/index');
    }

    public function uploaddocument($status,$url){
           $sina=M('oauth')->field('token')->where(array('code'=>'sina'))->find();
          $result=$this->_om->uploadDoc($sina['token'],$status,$url);
          dump($result);
          return $result;
    }
