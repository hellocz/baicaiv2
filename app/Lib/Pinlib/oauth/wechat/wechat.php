<?php
require_once dirname(__FILE__) . '/wechat.class.php'; //导入腾讯SDK
class wechat_oauth
{
    ...
    function getAuthorizeURL() {
        ...
        return $oauth->getAuthorizeURL($this->redirect_uri,"STATE");
    }
    ...
    public function getUserInfo($request_args) {
        ...
        $token = $oauth->getAccessToken($keys['code']);
        $user = $oauth->getUserInfo($token["access_token"], $token["openid"]);
        $result['keyid'] = $token["openid"];
